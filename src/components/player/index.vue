<template>
  <main class="bg-neutral text-neutral-content">
    <!-- Test -->
    <div id="container" class="grid grid-cols-3 text-sm grid-row-1">
      <!-- This is hacked I couldnt get it allign center vertically any other way then puttin it inside a div -->
      <div class="flex items-center justify-center">
        <div id="volume" class="w-20">
          <Slider v-model="volume" :step="-1" :tooltips="false" @update="volumeChange" />
        </div>
        <!--         <a
      ...
      <div id="playControls" class="col-start-2">
        <!-- Previous Button -->
        <button class="px-4 py-2 font-bold text-gray-800 co" @click="previosTrack">
          <font-awesome-icon icon="fast-backward" color="gray" />
        </button>
        <!-- Play Button -->
        <button class="px-4 py-2 font-bold text-gray-800 co" @click="playPause">
          <font-awesome-icon :icon="currentIcon" color="gray" />
        </button>
        <!-- Next Button -->
        <button class="px-4 py-2 font-bold text-gray-800 co" @click="nextTrack">
          <font-awesome-icon icon="fast-forward" color="gray" />
        </button>
      </div>
    ...
    <div id="barC" class="grid w-full pb-3 text-xs grid-row-1">
      <a>{{ eTime }} / {{ duration }}</a>
      <div id="playbar" class="mt-2 mb-2 ml-96 mr-96">
        <Slider v-model="percentPlayed" :step="-1" :tooltips="false" @update="barChange" />
      </div>
    </div>
    <!--     <AvCanvas
